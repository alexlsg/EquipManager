                ...
                if (_cmd != null)
                {
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(_eq.PORT))
                            _url += ":" + _eq.IP;
                        _cmdurl = $"http://{_url}/cgi-bin/webctrldev.cgi?device_id={_eq.NO}&cmd={_cmd.CMD}&cmdparam={_cmd.Param}&param_value={_cmd.ParamValue}&timeout={_cmd.TimeOut}&level={_cmd.Level}&IsCurve={_cmd.IsCurve}&cmd_notes={_cmd.CMDNotes}";
                    ...
                    {
                        _cmd.Result = $"{_eq.Name}执行失败命令{_cmd.CMD},命令参数:{_cmd.Param},命令参数值:{_cmd.ParamValue},命令说明:{_cmd.CMDNotes},错误信息:{ex.Message}";
                        Log.Add(ex);
                        Log.Add(_cmdurl);
                    }
